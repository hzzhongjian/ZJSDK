...
typedef SWIFT_ENUM(NSUInteger, FSAdLandStyle, open) {
  FSAdLandStyleNone = 0,
  FSAdLandStyleSheet = 1,
};

@class NSString;
SWIFT_CLASS("_TtC12FSUnionAdSDK13FSAdParamsKey")
@interface FSAdParamsKey : NSObject
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull landHeight;)
+ (NSString * _Nonnull)landHeight SWIFT_WARN_UNUSED_RESULT;
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull landStyle;)
+ (NSString * _Nonnull)landStyle SWIFT_WARN_UNUSED_RESULT;
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull landOutsideCancel;)
+ (NSString * _Nonnull)landOutsideCancel SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
@interface FSAdSlot (SWIFT_EXTENSION(FSUnionAdSDK))
/// 添加单个参数
- (void)addRequestParamWithKey:(NSString * _Nonnull)key value:(id _Nonnull)value;
/// 批量添加参数
- (void)addRequestParams:(NSDictionary<NSString *, id> * _Nonnull)params;
/// 移除参数
- (void)removeRequestParamForKey:(NSString * _Nonnull)key;
/// 获取参数
- (id _Nullable)getRequestParamForKey:(NSString * _Nonnull)key SWIFT_WARN_UNUSED_RESULT;
/// 清空所有参数（除了内置参数）
- (void)clearAllParams;
/// 清空所有参数（包括内置参数）
- (void)resetAllParams;
/// 设置自定义参数（链式调用支持）
- (nonnull instancetype)setParamWithKey:(NSString * _Nonnull)key value:(id _Nonnull)value;
@end
