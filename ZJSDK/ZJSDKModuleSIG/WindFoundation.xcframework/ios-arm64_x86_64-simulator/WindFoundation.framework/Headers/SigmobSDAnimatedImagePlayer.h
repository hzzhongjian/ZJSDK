...
@interface SigmobSDAnimatedImagePlayer : NSObject
...
/// The provider can be any protocol implementation, like `SigmobSDAnimatedImage`, `SigmobSDImageGIFCoder`, etc.
...
- (nullable instancetype)initWithProvider:(nonnull id<SigmobSDAnimatedImageProvider>)provider;

/// Create a player with animated image provider. If the provider's `animatedImageFrameCount` is less than 1, returns nil.
/// The provider can be any protocol implementation, like `SigmobSDAnimatedImage` or `SigmobSDImageGIFCoder`, etc.
/// @note This provider can represent mutable content, like progressive animated loading. But you need to update the frame count by yourself
/// @param provider The animated provider
+ (nullable instancetype)playerWithProvider:(nonnull id<SigmobSDAnimatedImageProvider>)provider;
