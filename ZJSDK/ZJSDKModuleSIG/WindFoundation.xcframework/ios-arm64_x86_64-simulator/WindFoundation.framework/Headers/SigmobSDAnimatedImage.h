...
#import "SigmobSDImageCoder.h"

/**
 This is the protocol for SigmobSDAnimatedImage class only but not for SigmobSDAnimatedImageCoder. If you want to provide a custom animated image class with full advanced function, you can conform to this instead of the base protocol.
 */
@protocol SigmobSDAnimatedImage <SigmobSDAnimatedImageProvider>
 ...
 */
- (nullable instancetype)initWithData:(nonnull NSData *)data scale:(CGFloat)scale options:(nullable SigmobSDImageCoderOptions *)options;

/**
 Initializes the image with an animated coder. You can use the coder to decode the image frame later.
 @note We use this with animated coder which conforms to `SigmobSDProgressiveImageCoder` for progressive animation decoding.
 
 @param animatedCoder An animated coder which conform `SigmobSDAnimatedImageCoder` protocol
 @param scale The scale factor to assume when interpreting the image data. Applying a scale factor of 1.0 results in an image whose size matches the pixel-based dimensions of the image. Applying a different scale factor changes the size of the image as reported by the `size` property.
 @return An initialized object
 */
- (nullable instancetype)initWithAnimatedCoder:(nonnull id<SigmobSDAnimatedImageCoder>)animatedCoder scale:(CGFloat)scale;
...
/**
 ...
 @note We use this with animated coder which conforms to `SigmobSDProgressiveImageCoder` for progressive animation decoding.
 */
@property (nonatomic, strong, readonly, nullable) id<SigmobSDAnimatedImageCoder> animatedCoder;
...
@interface SigmobSDAnimatedImage : UIImage <SigmobSDAnimatedImage>
...
// @note Before 5.19, these initializer will return nil for static image (when all candidate SigmobSDAnimatedImageCoder returns nil instance), like JPEG data. After 5.19, these initializer will retry for static image as well, so JPEG data will return non-nil instance. For vector image(PDF/SVG), always return nil.
// @note When the animated image frame count <= 1, all the `SigmobSDAnimatedImageProvider` protocol methods will return nil or 0 value, you'd better check the frame count before usage and keep fallback.
 ...
 */
@property (nonatomic, assign, readonly) SigmobSDImageFormat animatedImageFormat;
...
// By default, animated image frames are returned by decoding just in time without keeping into memory. But you can choose to preload them into memory as well, See the description in `SigmobSDAnimatedImage` protocol.
