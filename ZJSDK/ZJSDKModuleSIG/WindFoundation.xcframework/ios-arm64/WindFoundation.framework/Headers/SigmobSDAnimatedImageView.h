...
#import "SigmobSDAnimatedImage.h"
#import "SigmobSDAnimatedImagePlayer.h"
#import "SigmobSDImageTransformer.h"

/**
 A drop-in replacement for UIImageView/NSImageView, you can use this for animated image rendering.
 Call `setImage:` with `UIImage(NSImage)` which conforms to `SigmobSDAnimatedImage` protocol will start animated image rendering. Call with normal UIImage(NSImage) will back to normal UIImageView(NSImageView) rendering
 ...
 */
NS_SWIFT_UI_ACTOR
@interface SigmobSDAnimatedImageView : UIImageView
/**
 ...
 */
@property (nonatomic, strong, readonly, nullable) SigmobSDAnimatedImagePlayer *player;
 ...
 */
@property (nonatomic, strong, nullable) id<SigmobSDImageTransformer> animationTransformer;
...
@property (nonatomic, assign) SigmobSDAnimatedImagePlaybackMode playbackMode;
...
/**
 ...
 @note The best practice to use incremental load is using `initWithAnimatedCoder:scale:` in `SigmobSDAnimatedImage` with animated coder which conform to `SigmobSDProgressiveImageCoder` as well. Then call incremental update and incremental decode method to produce the image.
...
/**
 If the image which conforms to `SigmobSDAnimatedImage` protocol has more than one frame, set this value to `YES` will automatically
